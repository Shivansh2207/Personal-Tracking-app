...
interface AuthState {
  ...
  phase: AuthPhase;
  /** Set when the profile could not be loaded — the UI shows a retry state. */
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  init: () => {
    ...
    return subscribeToAuth(async (user) => {
      ...
      // Show the cached profile immediately so a returning user is never
      // bounced back to the login screen while the network call resolves.
      ...
      try {
        ...
        if (!profile) {
          profile = await createProfileIfMissing(user, user.displayName ?? '');
        }
        set({
          profile,
  ...
  refreshProfile: async () => {
    ...
    try {
      ...
      set({
        profileError: error instanceof Error ? error.message : 'Could not load your profile.',
      });
    }
  },
  ...
  finishOnboarding: async () => {
    ...
    set({
      phase: 'ready',
      profile: profile ? { ...profile, onboardingComplete: true } : profile,
    });
  },
