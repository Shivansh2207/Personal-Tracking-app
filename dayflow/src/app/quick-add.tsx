...
const OPTIONS: {
  ...
  { key: 'routine', label: 'Routine', caption: 'Something that repeats', icon: 'repeat', href: '/routine/new' },
  { key: 'focus', label: 'Study session', caption: 'Start a focus timer', icon: 'play', href: '/focus/setup' },
  { key: 'log', label: 'Log study', caption: 'Record study you already did', icon: 'edit-3', href: '/study/log' },
  { key: 'slot', label: 'Timetable slot', caption: 'Add to your weekly schedule', icon: 'calendar', href: '/timetable/new' },
  { key: 'note', label: 'Note', caption: 'Capture a thought', icon: 'file-text', mode: 'note' },
];

/**
 * Quick Add. Choosing "Task" drops the cursor straight into the name field, so
 * a task can be captured in a couple of seconds.
 */
export default function QuickAdd() {
  ...
  const saveTask = async () => {
    ...
    try {
      ...
      await createTask(uid, { title, dateKey: today });
  ...
  const saveNote = async () => {
    ...
    try {
      ...
      const { notesCol } = await import('@/services/firebase/paths');
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={{ gap: space.base, paddingBottom: space.base }}>
            <TextField
              ...
              placeholder="Submit the assignment"
              autoFocus
              returnKeyType="done"
              onSubmitEditing={saveTask}
            />
            <Body tone="faint" style={{ fontSize: 13 }}>
              Saved to today. Open it later to add a time, category or repeat.
            </Body>
