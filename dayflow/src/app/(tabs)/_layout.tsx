...
export default function TabsLayout() {
  ...
  return (
    <Tabs
      tabBar={(props) => <DayFlowTabBar {...props} />}
...
/**
 * Custom bar with the Quick Add action promoted into the middle, so capturing
 * anything is always two taps from anywhere in the app.
 */
function DayFlowTabBar({ state, navigation }: BottomTabBarProps) {
    ...
    <View
      ...
      <View style={{ width: 76, alignItems: 'center' }}>
        <Pressable
          ...
          accessibilityHint="Add a task, routine, study session, timetable slot or note"
          onPress={() => {
            Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium).catch(() => {});
            router.push('/quick-add');
          }}
          onLongPress={() => router.push('/quick-log')}
          style={({ pressed }) => ({
            width: 52,
            height: 52,
            marginTop: -18,
