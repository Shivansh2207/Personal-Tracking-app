...
function AppShell() {
  ...
  useEffect(() => {
    ...
    if (phase === 'signed_out' && !inAuth) router.replace('/(auth)/welcome');
    else if (phase === 'onboarding' && !inOnboarding) router.replace('/(onboarding)');
    else if (phase === 'ready' && (inAuth || inOnboarding)) router.replace('/(tabs)');
