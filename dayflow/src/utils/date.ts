...
/** Day part used to group the Today timeline. */
...
/**
 * Signed minutes between an actual clock time and a target.
 * Positive means later than target. Times that wrap past midnight (a 00:20
 * bedtime against a 23:30 target) are resolved to the nearest interpretation
 * rather than reported as a 23-hour deviation.
 */
export function clockDeviationMinutes(
...
/** "+14 min" / "−6 min" / "On target". */
export function formatDeviation(minutes: number | null, tolerance = 0): string {
  if (minutes === null) return '—';
  if (Math.abs(minutes) <= tolerance) return 'On target';
  const sign = minutes > 0 ? '+' : '−';
  return `${sign}${formatDuration(Math.abs(minutes), '0m')}`;
}

/** Averages a set of clock times, handling the midnight wrap for bedtimes. */
export function averageClockMinutes(times: (TimeString | null | undefined)[]): number | null {
  const values = times
    .map((t) => timeToMinutes(t))
    .filter((v): v is number => v !== null);
  if (values.length === 0) return null;
  // Late-night times (after 20:00) are shifted below zero so a 23:50 and a
  // 00:10 average to midnight rather than to midday.
  const shifted = values.map((v) => (v >= 20 * 60 ? v - 1440 : v));
  const spread = Math.max(...shifted) - Math.min(...shifted);
  const source = spread > 720 ? values : shifted;
  const mean = source.reduce((a, b) => a + b, 0) / source.length;
  return ((Math.round(mean) % 1440) + 1440) % 1440;
}
